// Package parsehtml will parse an html string into a html representation.
package parsehtml
...
func Must(str string) html.Node {
	n, err := Parse(str)
	if err != nil {
		panic(err)
	}
...
func (op *evalOp) eval(pn *tree.PN) {
	...
	case "DocType":
		op.cur.AddChildren(html.NewDoctype(pn.Value()))
	case "VoidTag":
		op.voidtag(pn)
	case "Tag":
		op.tag(pn)
...
func addAttributes(tag html.TagNode, attrs *tree.PN) {
